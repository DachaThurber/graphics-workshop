...
float dot = pow(min(dot(wo, r), 0.0), shininess);
vec3 specularity = dot * ks;
vec3 specular = specularity; // Change me!
...
void main() {
    ...
    // Generate temp shading based on distance to equator and elevation
    float tempFactor = abs((coord.y / 20.0) * elevation);
    vec3 tempColor = vec3(0.5, 0.2, 0.1);
    tempColor = mix(tempColor, vec3(0, 0.8, 0.8), smoothstep(0.0, 0.9, tempFactor));
    color = mix(color, tempColor, tempFactor);
